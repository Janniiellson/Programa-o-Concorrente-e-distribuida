public class Cliente extends Thread {
    ...
    public void run() {
        while (conta.getSaldo() > 0) {
            ...
            synchronized (conta) { // Sincronização para garantir operações seguras
                fazerCompra(valorCompra, lojas[0].getConta());
            }
    
            lojas[0].efetuarPagamentoSalariosSePossivel();
    ...
    public synchronized void fazerCompra(double valor, Conta contaDestino) {
        if (conta.getSaldo() >= valor) {
            banco.fazerTransferencia(conta, contaDestino, valor);
